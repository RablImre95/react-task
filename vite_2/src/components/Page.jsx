 ...
 if(searchString.length > 2) setFilteredArray([...data].filter(string => string.toLowerCase().includes(searchString.toLowerCase())))
  ...
  return (
    <div className="page">
      ...
      ? filteredArray.map((paragraph, index) => (<Paragraph key={index} index={index} string={paragraph} />))
      : data.map((paragraph, index) => (<Paragraph key={index} index={index} string={paragraph} />))}
    </div>
  );
}
